...
The macro automatically generates implementations for most zipper traits ([`Zipper`], [`ZipperMoving`], [`ZipperIteration`], [`ZipperValues`], [`ZipperValuesAt`], etc.) by dispatching each method call to the appropriate variant. Note that [`ZipperForking`] is intentionally not implemented, as the mapping between child zipper types and output types is not always straightforward and should be implemented manually when needed.
