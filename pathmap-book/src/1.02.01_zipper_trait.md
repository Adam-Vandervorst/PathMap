...
- [`fork_read_zipper`] creates a new read-only zipper with its root at the current focus. The returned zipper implements [`ZipperAbsolutePath`] + [`ZipperIteration`] + [`ZipperValues`] + [`ZipperValuesAt`].  This is useful for constructing child zippers with different roots, creating read zippers from other zipper types, or creating zippers to pass to functions that take ownership. However, cloning the parent zipper is often preferable when you need to maintain the original zipper.
