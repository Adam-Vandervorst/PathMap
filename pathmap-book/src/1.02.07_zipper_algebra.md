...
These operations return `Option<AlgebraicStatus>`: `None` means no element
remains within the destination, while `Some` reports whether the destination was changed or was an
...
`subtract_into` likewise returns `None` when subtraction removes the
destination entirely, and otherwise returns its status inside `Some`.
