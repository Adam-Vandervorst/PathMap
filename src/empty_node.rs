...
impl<V: Clone + Send + Sync, A: Allocator> TrieNode<V, A> for EmptyNode {
    ...
    fn psubtract_dyn(&self, _other: TaggedNodeRef<V, A>) -> Option<AlgebraicResult<TrieNodeODRc<V, A>>> where V: DistributiveLattice {
        None
    }
    fn prestrict_dyn(&self, _other: TaggedNodeRef<V, A>) -> Option<AlgebraicResult<TrieNodeODRc<V, A>>> {
        None
    }
