...
where Storage: AsRef<[u8]>
{
    ...
    #[cold]
    #[inline(never)]
    fn to_sibling_from_nonexistent_path(&mut self, next: bool) -> bool {
        ...
        let sibling_idx = match &self.cur_node {
            Node::Line(line) => {
                let frame = self.stack.last().unwrap();
                let Some(&byte) = self.tree.get_line(line.path).get(frame.node_depth) else {
                    return false;
                };
            ...
            Node::Branch(node) => {
                let sibling_byte = if next {
                    node.bytemask.next_bit(cur_byte)
                } else {
                    node.bytemask.prev_bit(cur_byte)
                };
                let Some(byte) = sibling_byte else {
                    return false;
                };
                node.bytemask.index_of(byte) as usize
            }
        };
...
where Storage: AsRef<[u8]>
{
    ...
    fn descend_first_k_path(&mut self, k: usize) -> bool {
        ...
        loop {
            while depth < k {
                if self.descend_first_byte() { depth += 1; } else { break; }
            }
            if depth == k {
                return true;
            }
...
mod tests {
    ...
    fn act_zipper_sibling_step_from_an_off_trie_focus() {
        ...
        az.descend_to(&[2u8]);
        assert!(!az.path_exists());
        assert!(az.to_next_sibling_byte());
        assert_eq!(az.path(), &[3u8]);
        assert_eq!(az.val(), Some(&7));
        az.ascend(1);
        az.descend_to(&[2u8]);
        assert!(az.to_prev_sibling_byte());
