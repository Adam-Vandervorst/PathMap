...
/// WIP
pub fn deserialize_fork<V: TrieValue + 'static, A: Allocator, WZ : ZipperWriting<V, A> + zipper::ZipperMoving + zipper::ZipperPath, F: Fn(usize, &[u8]) -> V>(node: usize, wz: &mut WZ, source: &[u8], fv: F) -> std::io::Result<usize> {
    unsafe {
    // let mut recovered = 0;
