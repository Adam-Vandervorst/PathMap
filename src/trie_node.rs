...
/// The node holding the rest of `key` after following onward links, and that rest
#[inline]
...
/// Drops dangling paths below `node`; nodes shared with `src` are skipped
...
/// Meets one list-node slot against `other`.  Returns `(self_ident, counter_ok, contribution)`
...
/// Whether all of `other` lies within the met slots
...
mod tests {
    ...
    /// A meet keeps the left operand's value, whatever the node types
    #[test]
    ...
    /// `join_k_path_into` keeps the first k-path's value on a collision
    #[test]
